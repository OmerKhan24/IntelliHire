...
class Job(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'title': self.title, 'description': self.description, 'requirements': self.requirements, 'duration_minutes': self.duration_minutes, 'created_by': self.created_by, 'status': self.status, 'scoring_criteria': self.scoring_criteria or {}, 'created_at': self.created_at.isoformat() if self.created_at else None, 'updated_at': self.updated_at.isoformat() if self.updated_at else None}
...
class Interview(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'job_id': self.job_id, 'candidate_name': self.candidate_name, 'candidate_email': self.candidate_email, 'candidate_phone': self.candidate_phone, 'status': self.status, 'started_at': self.started_at.isoformat() if self.started_at else None, 'completed_at': self.completed_at.isoformat() if self.completed_at else None, 'final_score': self.final_score, 'ai_analysis': self.ai_analysis or {}, 'recording_url': self.recording_url, 'created_at': self.created_at.isoformat() if self.created_at else None, 'updated_at': self.updated_at.isoformat() if self.updated_at else None}
...
class Response(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'interview_id': self.interview_id, 'question_id': self.question_id, 'answer_text': self.answer_text, 'answer_audio_url': self.answer_audio_url, 'answer_video_url': self.answer_video_url, 'answer_duration': self.answer_duration, 'confidence_score': self.confidence_score, 'relevance_score': self.relevance_score, 'technical_score': self.technical_score, 'communication_score': self.communication_score, 'ai_feedback': self.ai_feedback, 'detected_emotions': self.detected_emotions or {}, 'behavioral_flags': self.behavioral_flags or {}, 'created_at': self.created_at.isoformat() if self.created_at else None}
