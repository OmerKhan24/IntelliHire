    ...
    async def generate_questions(self, job_description, requirements, num_questions=5, cv_text=None, duration_minutes=20, scoring_criteria=None):
        ...
        try:
            if cv_text:
                ...
                user_prompt = f"""You are an expert technical interviewer conducting a {duration_minutes}-minute interview.
...
🎯 SCORING FOCUS:
- Technical Skills: {technical_weight}%
- Behavioral/Soft Skills: {behavioral_weight}%  
- Situational/Problem Solving: {situational_weight}%
...
1. REFERENCE THEIR CV: Ask about specific projects, technologies, or experiences mentioned in their CV
2. ASSESS CLAIMED SKILLS: Test depth of knowledge in technologies/skills they list
3. MATCH SCORING WEIGHTS: Distribute questions according to the scoring percentages above
4. APPROPRIATE DEPTH: Questions should require {time_per_question}-minute answers
5. PROGRESSIVE DIFFICULTY: Start easier, increase complexity
6. ROLE-SPECIFIC: Directly relate to the job requirements

IMPORTANT: Return ONLY the numbered questions (1-{num_questions}), one per line. NO additional text.

Example format:
1. I see you worked on [specific project from CV]. Can you explain the technical challenges you faced with [specific technology] and how you solved them?
2. [Next question]..."""
            else:
...
Generate EXACTLY {num_questions} professional interview questions that:

1. MATCH SCORING WEIGHTS: Distribute according to percentages above
2. APPROPRIATE DEPTH: Questions should require {time_per_question}-minute answers
3. PROGRESSIVE DIFFICULTY: Start with foundational questions, increase complexity
4. ROLE-SPECIFIC: Directly test skills needed for this position
5. ACTIONABLE: Require specific examples and technical details

IMPORTANT: Return ONLY the numbered questions (1-{num_questions}), one per line. NO additional text.

Example format:
1. Can you walk me through your experience with [key requirement]? What specific projects have you completed?
2. [Next question]..."""
            ...
            messages = [
                {"role": "system", "content": "You are an expert technical interviewer. Generate interview questions based on job requirements and candidate background."},
